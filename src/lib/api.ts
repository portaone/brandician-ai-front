...
export const brands = {
  ...
  get: async (brandId: string, currentApi: AxiosInstance = api) => {
    const key = createRequestKey("GET", apiPath(`/brands/${brandId}`));
    return deduplicate(key, async () => {
      const response = await currentApi.get(apiPath(`/brands/${brandId}`));
      return response.data;
    });
  },
  ...
  updateStatus: async (brandId: string, status: BrandStatus) => {
    const response = await api.put(apiPath(`/brands/${brandId}/status`), {
      status,
    });
    return response.data;
  },
  ...
  revertToStatus: async (brandId: string, targetStatus: BrandStatus) => {
    const requestId = generateRequestId();
    logApiCall("POST", apiPath(`/brands/${brandId}/revert/`), requestId, {
      target_status: targetStatus,
    });
    const response = await api.post(apiPath(`/brands/${brandId}/revert/`), {
      target_status: targetStatus,
    });
    logApiCall(
      "POST-RESPONSE",
  ...
  processAudio: async (brandId: string, answerId: string, audioFile: File) => {
    ...
    const response = await api.post(
      apiPath(`/brands/${brandId}/answers/${answerId}/audio`),
      formData,
      {
        headers: {
  ...
  processAudioBatch: async (
    ...
    formData.append("audio_file", audioFile);
    const response = await api.post(
      apiPath(`/brands/${brandId}/answers/${answerId}/audio-batch`),
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
  ...
  listAssets: async (brandId: string, currentApi: AxiosInstance = api) => {
