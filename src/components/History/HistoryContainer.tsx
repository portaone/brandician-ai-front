...
const HistoryContainer: React.FC = () => {
  ...
  const { currentBrand, selectBrand, isLoading: brandLoading } = useBrandStore();
  ...
  const currentStepNumber = currentBrand ? getCurrentStepNumber(currentBrand.current_status || '') : 0;
  ...
  const steps: HistoryStep[] = [
    {
      ...
      dataLoader: async () => {
        ...
        const questions = Array.isArray(questionsResponse) ? questionsResponse : (questionsResponse?.questions || []);
  ...
  const handleRevertConfirm = async () => {
    ...
    try {
      ...
      alert(`Failed to revert: ${error.response?.data?.message || error.message || 'Unknown error'}`);
    } finally {
    ...
    if (isLoading) {
      return (
        <div className="p-4 text-center">
          <div className="animate-spin text-primary-600 text-xl inline-block">⟳</div>
    ...
    switch (data.type) {
          ...
          return data.questions.map((q: any, idx: number) => {
            ...
            if (data.answers) {
              if (Array.isArray(data.answers)) {
                answer = data.answers.find((a: any) =>
                  a.question_id === q.id ||
            ...
            const answerText = answer ? (answer.answer || answer.answer_text || 'No answer provided') : 'No answer recorded';
            ...
            {data.questions && Array.isArray(data.questions) ? (
              <ol className="space-y-4 list-none">
                {data.questions.map((q: any, idx: number) => {
                  ...
                  return (
                    <li key={idx} className="border-l-4 border-primary-300 pl-4 py-3">
                      <div className="flex gap-3">
                        <span className="font-bold text-primary-600 flex-shrink-0">
                          {idx + 1}.
                        </span>
                        <div className="flex-1">
                          <p className="font-medium text-gray-900 mb-2">{q.text || q.question_text || q.question}</p>
                          {answer ? (
                            <p className="text-gray-700 bg-gray-50 p-3 rounded">
                              {answer.answer || answer.answer_text || 'No answer provided'}
                            </p>
                          ) : (
                            <p className="text-gray-500 italic text-sm">No answer recorded</p>
                          )}
                        </div>
                      </div>
                    </li>
              ...
              <div className="bg-gray-50 p-4 rounded">
                <pre className="whitespace-pre-wrap text-sm text-gray-700">
                  {JSON.stringify({ questions: data.questions, answers: data.answers }, null, 2)}
                </pre>
              </div>
            )}
          </div>
      ...
      case 'jtbd':
        ...
        const formatJTBD = () => {
          ...
          // Add personas
          if (data.data.personas && Object.keys(data.data.personas).length > 0) {
            text += 'PERSONAS:\n\n';
            Object.entries(data.data.personas).forEach(([key, persona]: [string, any]) => {
              text += `${persona.name}\n${persona.description}\n\n`;
            });
          }
      ...
      case 'survey':
        ...
        const surveyQuestions = data.data?.results?.questions ||
                                data.data?.questions ||
                                data.questions ||
                                [];
        ...
        const formatSurveyQuestions = () => {
          ...
          return surveyQuestions.map((q: any, idx: number) => {
            let text = `${idx + 1}. ${q.text || q.question || 'Question'}\nType: ${q.type || 'text'}`;
          ...
          <div className="p-4 space-y-3">
            {surveyUrl && (
              <div className="mb-4 p-3 bg-blue-50 rounded border border-blue-200">
                <h4 className="font-semibold text-gray-900 mb-2">Survey URL:</h4>
                <a
                  href={surveyUrl}
                  target="_blank"
              ...
              <div className="space-y-2">
                {surveyQuestions.map((q: any, idx: number) => (
                  <div key={idx} className="bg-gray-50 p-3 rounded">
                    <p className="font-medium text-gray-800">{q.text || q.question || 'Question'}</p>
                    <p className="text-sm text-gray-500 mt-1">Type: {q.type || 'text'}</p>
                    {q.options && q.options.length > 0 && (
                      <div className="mt-2">
                        <p className="text-sm text-gray-600">Options:</p>
                        <ul className="list-disc list-inside text-sm text-gray-600 ml-2">
                          {q.options.map((opt: any, optIdx: number) => (
                            <li key={optIdx}>{typeof opt === 'string' ? opt : opt.text || opt.value}</li>
                          ))}
                        </ul>
                      </div>
                    )}
      ...
      case 'feedback':
        const handleDownloadCsv = async () => {
          ...
          try {
            ...
            a.href = url;
            a.download = `${currentBrand?.name || 'brand'}_survey_responses.csv`;
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="bg-gray-50 p-4 rounded">
              <p className="text-gray-700">
                Responses: {data.data?.number_of_responses || 0} / {data.data?.min_responses_required || 0}
              </p>
        ...
        return (
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              <div className="bg-gray-50 p-4 rounded">
                <p className="text-gray-700">
                  {data.archetype?.archetype || data.archetype || 'No archetype available'}
                </p>
              </div>
            </div>
          </div>
      ...
      case 'summary':
        const summaryText = data.data?.summary || data.data || 'No summary available';
        return (
          <div className="p-4 space-y-3">
            <div className="flex items-center justify-between mb-2">
      ...
      case 'archetype':
        const archetypeText = data.data?.archetype || data.data || 'No archetype available';
        return (
          <div className="p-4 space-y-3">
            <div className="flex items-center justify-between mb-2">
      ...
      case 'name_selection':
        return (
          <div className="p-4 space-y-3">
            <h4 className="font-semibold text-gray-900">Selected Brand Name:</h4>
            <div className="bg-gray-50 p-4 rounded">
            ...
            {data.data?.assets && Array.isArray(data.data.assets) ? (
              <div className="space-y-3">
                {data.data.assets.map((assetSummary: BrandAssetSummary) => {
                  ...
                  return (
                    <div key={assetSummary.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <div className="pt-4 space-y-4">
                              {loadedAsset.description && (
                                <div>
                                  ...
                                  <p className="text-gray-600 text-sm">{loadedAsset.description}</p>
                                </div>
                              )}
                              ...
                              {loadedAsset.url && (
                                <div>
                                  <h6 className="font-medium text-gray-700 mb-1">URL:</h6>
                                  <a
                              ...
                              {loadedAsset.created_at && (
                                <div className="text-xs text-gray-500 pt-2 border-t border-gray-100">
                                  Created: {new Date(loadedAsset.created_at).toLocaleString()}
                                </div>
                              )}
                            </div>
                          )}
      ...
      case 'testimonial':
        return (
          <div className="p-4 space-y-3">
            <h4 className="font-semibold text-gray-900 mb-2">User Testimonial:</h4>
            {data.data ? (
              <div className="bg-gray-50 p-4 rounded space-y-3">
                {data.data.rating && (
                  <div>
                    <p className="text-sm text-gray-600">Rating:</p>
                    <p className="text-yellow-500 text-xl">{'★'.repeat(data.data.rating)}{'☆'.repeat(5 - data.data.rating)}</p>
                  </div>
                )}
                {data.data.testimonial && (
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Public Testimonial:</p>
                    <p className="text-gray-700">{data.data.testimonial}</p>
                  </div>
                )}
                {data.data.feedback && (
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Internal Feedback:</p>
                ...
                {data.data.amount && (
                  <div>
                    ...
                    <p className="text-gray-700 font-medium">${data.data.amount}</p>
                  </div>
                )}
              </div>
            ) : (
          ...
          <div className="space-y-4">
            {steps.map((step) => {
              ...
              return (
                <div
                  key={step.number}
                  className={`bg-white rounded-lg shadow-md overflow-hidden ${!isCompleted && !isActive ? 'opacity-60' : ''}`}
                >
                  {/* Step Header */}
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-6 transition-colors ${
                      isCompleted ? 'hover:bg-gray-50 cursor-pointer' : isActive ? 'cursor-default' : 'cursor-not-allowed'
                    }`}
                  >
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              if (currentBrand && brandId) {
                                ...
                                console.error('❌ Missing currentBrand or brandId:', { currentBrand, brandId });
                              }
                            }}
      ...
      {revertModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Confirm Revert</h2>
            <p className="text-gray-700 mb-6">
              All assets and documents generated after this step will be lost. Are you sure you want to revert to step {revertTargetStep}?
            </p>
