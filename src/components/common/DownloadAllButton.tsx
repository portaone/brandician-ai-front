  ...
  const handleDownloadAll = async () => {
    try {
      ...
      for (const asset of assets) {
        try {
          const fullAsset = await brands.getAsset(brandId, asset.id, guestApi);
      ...
      for (const [assetType, asset] of Object.entries(assetContents)) {
        if (asset && asset.content) {
          ...
          try {
            ...
            if (
              asset.content.includes("http") &&
              asset.content.includes("\n")
            ) {
              fileExtension = ".txt";
            } else if (
              asset.content.includes("#") &&
              asset.content.includes("rgb")
            ) {
              fileExtension = ".css";
            }
...
`;
  ...
  if (variant === "link") {
    return (
      ...
      >
        <Download
          className={`h-4 w-4 mr-1 ${isDownloading ? "animate-spin" : ""}`}
        />
