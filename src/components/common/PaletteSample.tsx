...
/** Normalise parsed JSON into an array of palette variant objects. */
function normalisePalettes(raw: unknown): PaletteColors[] {
  if (Array.isArray(raw)) return raw as PaletteColors[];
  if (raw && typeof raw === "object") return [raw as PaletteColors];
  return [];
}
...
const PaletteSample: React.FC<PaletteSampleProps> = ({
  ...
  let palettes: PaletteColors[] = [];
  try {
    palettes = normalisePalettes(JSON.parse(content));
  } catch (e) {
  ...
  return (
    <>
      {palettes.map((colors, idx) => {
        ...
        const variantName = colors.name || `Variant ${idx + 1}`;

        return (
          <div key={idx} style={{ marginBottom: palettes.length > 1 ? 32 : 0 }}>
            {/* Variant heading */}
            {palettes.length > 1 && (
              <h3
                style={{
            ...
            <div
              onClick={() => handleOpenColorSchema(idx)}
              style={{
                cursor: "pointer",
            ...
            {onSelect && (
              <div style={{ marginTop: 16, paddingLeft: 8 }}>
                <Button
                  ...
                  size="lg"
                  loading={isSaving}
                  leftIcon={!isSaving && <Check className="h-5 w-5" />}
                >
                  {isSaving ? "Saving..." : "Select this palette"}
                </Button>
              </div>
            )}
          </div>
