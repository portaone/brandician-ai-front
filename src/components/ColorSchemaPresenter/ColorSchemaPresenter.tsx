...
const ColorSchemaPresenter: React.FC = () => {
    ...
    const loadBrandColors = async () => {
      if (currentBrand && brandId) {
        try {
          ...
          console.log('Available asset types:', response.assets.map((a: any) => a.type));

          // Find color palette asset
          const colorAsset = response.assets.find((asset: any) =>
            asset.type === 'palette' ||
            ...
            if (fullColorAsset.content) {
              try {
                ...
                lines.forEach((line: string) => {
                  ...
                  const colorMatch = line.match(/(primary|main|brand|supporting|secondary|background|accent|cta|action|text|foreground)[:\s]+#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})/i);
                  if (colorMatch) {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-white" style={{ minHeight: '1024px' }}>
      ...
      <div className="relative mx-auto" style={{ width: '1440px', height: '1024px' }}>
        ...
        <div className="absolute" style={{ left: '97px' }}>
          ...
          >
            Subtitle: supports the title with<br />
          ...
          >
            <p style={{ margin: '0 0 16px 0', fontSize: '16px', lineHeight: '24px' }}>
              This is the Body / Paragraph Text, used for presenting the main
              content of your brand — articles, service descriptions, blog posts,
              product info, and more. This text should feel easy on the eyes, with
              clear hierarchy and enough spacing to invite longer reading.
            </p>
            <p style={{ margin: 0, fontSize: '16px', lineHeight: '24px' }}>
              It's designed for clarity and flow — so the message is not just seen, but
              understood. Whether your tone is conversational or formal, this is
              where your voice truly lives.
            </p>
          </div>
        </div>
